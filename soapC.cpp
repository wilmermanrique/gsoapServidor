...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.55 2017-12-01 22:25:57 GMT")
...
{	switch (p->type)
	...
	case SOAP_TYPE__base64:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct _base64*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct _base64*>(p->ptr));
		break;
	case SOAP_TYPE__struct:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct _struct*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct _struct*>(p->ptr));
		break;
	case SOAP_TYPE_data:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct data*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct data*>(p->ptr));
		break;
	case SOAP_TYPE__array:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct _array*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct _array*>(p->ptr));
		break;
	case SOAP_TYPE_std__string:
		if (p->size < 0)
	...
	case SOAP_TYPE_value:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct value*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct value*>(p->ptr));
		break;
	case SOAP_TYPE_member:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct member*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct member*>(p->ptr));
		break;
	case SOAP_TYPE_params:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct params*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct params*>(p->ptr));
		break;
	case SOAP_TYPE_param:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct param*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct param*>(p->ptr));
		break;
	case SOAP_TYPE_methodResponse:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct methodResponse*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct methodResponse*>(p->ptr));
		break;
	case SOAP_TYPE_methodCall:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct methodCall*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct methodCall*>(p->ptr));
		break;
	case SOAP_TYPE_fault:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct fault*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct fault*>(p->ptr));
		break;
#ifndef WITH_NOGLOBAL
	case SOAP_TYPE_SOAP_ENV__Header:
		if (p->size < 0)
...
SOAP_FMAC1 struct methodCall * SOAP_FMAC2 soap_instantiate_methodCall(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct methodCall);
		if (p)
			((struct methodCall*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct methodCall, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct methodCall*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct methodCall location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_methodCall, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct fault * SOAP_FMAC2 soap_instantiate_fault(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct fault);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct fault, n);
		k *= n;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct fault location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_fault, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct methodResponse * SOAP_FMAC2 soap_instantiate_methodResponse(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct methodResponse);
		if (p)
			((struct methodResponse*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct methodResponse, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct methodResponse*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct methodResponse location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_methodResponse, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct param * SOAP_FMAC2 soap_instantiate_param(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct param);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct param, n);
		k *= n;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct param location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_param, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct params * SOAP_FMAC2 soap_instantiate_params(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct params);
		if (p)
			((struct params*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct params, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct params*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct params location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_params, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct _array * SOAP_FMAC2 soap_instantiate__array(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct _array);
		if (p)
			((struct _array*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct _array, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct _array*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct _array location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE__array, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct data * SOAP_FMAC2 soap_instantiate_data(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct data);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct data, n);
		k *= n;
	}
...
SOAP_FMAC1 struct member * SOAP_FMAC2 soap_instantiate_member(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct member);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct member, n);
		k *= n;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct member location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_member, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct _struct * SOAP_FMAC2 soap_instantiate__struct(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct _struct);
		if (p)
			((struct _struct*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct _struct, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct _struct*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct _struct location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE__struct, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 void SOAP_FMAC2 soap_default__base64(struct soap *soap, struct _base64 *a)
{
...
SOAP_FMAC1 struct _base64 * SOAP_FMAC2 soap_instantiate__base64(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct _base64);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct _base64, n);
		k *= n;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct _base64 location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE__base64, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
...
SOAP_FMAC1 struct value * SOAP_FMAC2 soap_instantiate_value(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (n < 0)
	{	p = SOAP_NEW(struct value);
		if (p)
			((struct value*)p)->soap = soap;
	}
	else
	{	p = SOAP_NEW_ARRAY(struct value, n);
		k *= n;
		if (p)
			for (int i = 0; i < n; i++)
				((struct value*)p)[i].soap = soap;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct value location=%p n=%d\n", (void*)p, n));
	soap_link(soap, p, SOAP_TYPE_value, n, soap_fdelete);
	if (size)
		*size = k;
	return p;
}
